  ...
  <div class="grid lg:grid-cols-2 min-h-screen">
    ...
    <section class="dark-panel flex flex-col justify-between gap-10 p-8 lg:p-14 order-2 lg:order-1">
      ...
      <div class="flex flex-col gap-5 max-w-md">
        <h1 class="text-[28px] lg:text-[38px] font-extrabold text-ondark leading-[1.4]">
          ورود به پنل مراکز و ادمین
        </h1>
        <p class="text-[15px] leading-[2] text-ondark-2">
        ...
        <div class="grid grid-cols-3 gap-4 mt-2">
          <div v-for="stat in [
            ...
            <span class="text-[24px] font-extrabold text-gold-400">{{ stat.value }}</span>
            <span class="text-[12px] text-ondark-3">{{ stat.label }}</span>
          </div>
        </div>
      </div>

      <p class="text-[14px] text-ondark-3">
    ...
    <section class="flex items-center justify-center p-8 lg:p-14 order-1 lg:order-2">
      <form class="w-full max-w-[420px] flex flex-col gap-6" novalidate @submit.prevent="submit">
        ...
        <div class="grid grid-cols-2 p-1 rounded-[999px]" style="background: var(--color-cream-200)">
          <button
            ...
            :class="tab === option.value ? 'bg-surface text-ink-900 shadow-sm' : 'text-muted'"
        ...
        <label class="flex items-center gap-3 text-[14px] cursor-pointer">
          <input v-model="remember" type="checkbox" class="w-[17px] h-[17px] rounded-[5px] accent-[#B24A2E]">
          مرا به خاطر بسپار
        </label>
        ...
        <p class="text-[13px] leading-7 text-muted text-center">
          ...
          <NuxtLink to="/contact" class="text-brick-500 hover:text-brick-600">درخواست ایجاد حساب</NuxtLink>
        </p>

        <NuxtLink to="/" class="text-[13px] text-muted hover:text-brick-500 text-center">
          ← بازگشت به صفحه اصلی
        </NuxtLink>
      </form>
    </section>
